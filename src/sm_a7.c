...
void Kraid_Init(void) {  // 0xA7A959
  ...
  unpause_hook = (LongPtr)LONGPTR(fnUnpauseHook_Kraid_IsAlive);
  pause_hook = (LongPtr)LONGPTR(fnPauseHook_Kraid);
...
void Kraid_GetsBig_BreakCeilingPlatforms(void) {  // 0xA7AC4D
  ...
  if ((E->base.y_pos & 3) != 0 || (kraid_var_F = E->kraid_var_F, (int16)(kraid_var_F - 18) >= 0)) {
      // @todo: change the if else to just if
  }
  else {
    ...
    case 0:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 6, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7B3_Kraid_CrumbleCeilingBlockIntoBg3 });
      break;
    case 1:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 13, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AB_Kraid_CrumbleCeilingBlockIntoBg2 });
      break;
    case 2:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 2, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7A3_Kraid_CrumbleCeilingBlockIntoBg1 });
      break;
    case 3:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 10, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7B3_Kraid_CrumbleCeilingBlockIntoBg3 });
      break;
    case 4:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 5, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AB_Kraid_CrumbleCeilingBlockIntoBg2 });
      break;
    case 5:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 12, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7B3_Kraid_CrumbleCeilingBlockIntoBg3 });
      break;
    case 6:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 3, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AB_Kraid_CrumbleCeilingBlockIntoBg2 });
      break;
    case 7:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 11, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AB_Kraid_CrumbleCeilingBlockIntoBg2 });
      break;
    case 8:
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 4, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7B3_Kraid_CrumbleCeilingBlockIntoBg3 });
      break;
    }
    E->kraid_var_F += 2;
  }
...
void Kraid_Enemy_Touch(void) {  // 0xA7B0F3
  ...
  if (sign16(E->kraid_var_A + 0x3CA0)) {
    ...
    for (i = 0; ; i += 4) {
      int v3 = i >> 1;
      if ((int16)(v1 - kKraid_BodyHitboxTable[v3]) >= 0 || (int16)(v1 - kKraid_BodyHitboxTable[v3 + 2]) >= 0)
        break;
    }
    if ((int16)(E->base.x_pos + kKraid_BodyHitboxTable[(i >> 1) + 1] - r18) < 0) {
    ...
    if (projectile_counter) {
      for (int i = 2 * projectile_counter; i >= 0; i -= 2) {
        ...
        if (sign16(projectile_y_pos[v5] - projectile_y_radius[v5] - 1 - r18)) {
          ...
          for (j = 0; ; j += 4) {
            int v13 = j >> 1;
            if ((int16)(v11 - kKraid_BodyHitboxTable[v13]) >= 0 || (int16)(v11 - kKraid_BodyHitboxTable[v13 + 2]) >= 0)
              break;
          }
          if ((int16)(E->base.x_pos + kKraid_BodyHitboxTable[(j >> 1) + 1] - r18) < 0) {
LABEL_7:
...
void Kraid_InitEyeGlowing(void) {  // 0xA7B6BF
  ...
  E->kraid_var_C = kKraid_EyeGlowingInstrLists[0].timer;
  ...
  if (!(E->base.properties & kEnemyProps_Intangible) && !samus_invincibility_timer) {
    ...
    if (!sign16(samus_x_radius + samus_x_pos - r18)) {
      if (sign16(samus_x_pos - samus_x_radius - r18)) {
        uint16 r22 = kKraid_LintBoundary.top + E->base.y_pos + 2;
        if (!sign16(samus_y_radius + samus_y_pos - r22)) {
          uint16 r24 = kKraid_LintBoundary.bottom + E->base.y_pos - 2;
...
void KraidsFoot_SecondPhase_Thinking(void) {  // 0xA7BA2E
  ...
  if (!v1) {
    ...
    while (1) {
      E0 = Get_Kraid(0);
      if (E0->base.x_pos == kKraid_XMovementTable[v2 >> 1])
        break;
      v2 += 4;
      if ((int16)(v2 - 24) >= 0) {
        v2 = 4;
    ...
    if (!sign16(v4 - 16))
      v4 = 16;
    const uint16 *v5 = (const uint16 *)RomPtr_A7(kKraid_XMovementTable[(v2 >> 1) + 1] + v4);
    uint16 v6 = v5[1];
...
void KraidsFingernail_Init(void) {  // 0xA7BD60
  ...
  if (sign16(kraid_var_E))
    v2 = kKraidsFingernail_UpwardsVelocityTable[(random_number & 6) >> 1];
  else
    v2 = kKraidsFingernail_DownwardsVelocityTable[(random_number & 6) >> 1];
...
void KraidsFingernail_Fire(uint16 k) {  // 0xA7BE8E
  ...
  if (Enemy_MoveRight_IgnoreSlopes(k, __PAIR32__(v1->ai_var_C, v1->ai_var_B))) {
    ...
    for (i = 0; ; i += 4) {
      v3 = gEnemyData(0);
      v4 = gEnemyData(k);
      if ((int16)(kKraidsFingernail_BoundaryOffsets[(i >> 1) + 1] + v3->y_pos - v4->y_pos) < 0)
        break;
    }
    uint16 r18 = kKraidsFingernail_BoundaryOffsets[i >> 1] + v3->x_pos;
...
void Kraid_SpawnPlmToClearCeiling(void) {  // 0xA7C168
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 2, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7B7_Kraid_ClearCeilingBlocks });
}

void Kraid_ClearSomeSpikes(void) {  // 0xA7C171
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 5, .y_pos = 27, .plm_id_ = addr_kPlmHeader_B7BB_Kraid_ClearSpikeBlocks });
}
...
void Kraid_Death_Init(void) {  // 0xA7C360
  ...
  if (!E0->kraid_hurt_frame) {
    ...
    E0->kraid_var_C = kKraid_DyingInstrLists[0].timer;
    ...
    cur_enemy_index = v7;
    SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 5, .y_pos = 27, .plm_id_ = addr_kPlmHeader_B7BF_Kraid_CrumbleSpikeBlocks });
  }
}
...
void Kraid_Death_UpdateBG2TilemapBottomHalf(void) {  // 0xA7C4C8
  Kraid_ProcessKraidInstr();
  unpause_hook = (LongPtr)LONGPTR(fnKraid_UnpauseHook_IsSinking);
  Enemy_Kraid *E = Get_Kraid(0);
...
void Kraid_CrumbleLeftPlatform_Left(void) {  // 0xA7C691
  ...
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 7, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7A7_Kraid_SetCeilingBlockToBg2 });
}
...
void Kraid_CrumbleRightPlatform_Middle(void) {  // 0xA7C6A7
  ...
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 15, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7A7_Kraid_SetCeilingBlockToBg2 });
}
...
void Kraid_CrumbleRightPlatform_Left(void) {  // 0xA7C6BD
  ...
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 14, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AF_Kraid_SetCeilingBlockIntoBg3 });
}
...
void Kraid_CrumbleLeftPlatform_Right(void) {  // 0xA7C6D3
  ...
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 9, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7A7_Kraid_SetCeilingBlockToBg2 });
}
...
void Kraid_CrumbleLeftPlatform_Middle(void) {  // 0xA7C6E9
  ...
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 8, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AF_Kraid_SetCeilingBlockIntoBg3 });
}
...
void Kraid_CrumbleRightPlatform_Right(void) {  // 0xA7C6FF
  ...
  SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 16, .y_pos = 18, .plm_id_ = addr_kPlmHeader_B7AF_Kraid_SetCeilingBlockIntoBg3 });
}
...
void Phantoon2_Init(void) {  // 0xA7CE55
  ...
  v6->current_instruction = kPhantoon_InstrListPtrs[v6->parameter_2];
...
void Phantoon_Func_1(void) {  // 0xA7CEED
  QueueSfx2_Max6(kPhantoon_MaterialisationSoundEffects[g_word_7E9032]);
...
void Phantoon_Func_6(uint16 k, uint16 a) {  // 0xA7CF8B
  ...
  v2 = kEproj_DestroyableFireballs_Positions[a];
  int n = 7;
  int r20 = 16;
  do {
    v3 = v2;
...
void Phantoon_Func_7(uint16 k) {  // 0xA7CFCA
  ...
  if (v3 || v4) {
    if ((E->phant_var_C & 0x8000) == 0) {
      ...
      if (v3 || v8) {
        E->phant_var_C = -1;
        E->phant_var_B = *((uint16 *)RomPtr_A7(kPhantoon_CasualFlameTimerPtrs[E->phant_var_A]) + 1);
      } else {
        E->phant_var_B = *((uint16 *)RomPtr_A7(2 * E->phant_var_C + kPhantoon_CasualFlameTimerPtrs[E->phant_var_A]) + 1);
      }
      E->base.instruction_timer = 1;
      E->base.current_instruction = addr_kPhantoon_MouthSpawnFlame_InstrLists;
    } else {
      uint16 v5 = NextRandom() & 3;
      E->phant_var_A = v5;
      uint16 v6 = kPhantoon_CasualFlameTimerPtrs[v5];
...
void Phantoon_PickPatternForRound2(void) {  // 0xA7D076
  ...
  Enemy_Phantoon *E = Get_Phantoon(0);
  E->phant_var_E = 60;
  uint16 v1 = kPhantoon_EyeClosedTimers[NextRandom() & 7];
  Enemy_Phantoon *E1 = Get_Phantoon(0x40);
  E1->phant_var_A = v1;
...
void Phantoon_ChangeEyeSpriteBasedOnSamusDist(void) {  // 0xA7D3FA
  ...
  E->base.instruction_timer = 1;
  E->base.current_instruction = kPhantoon_EyeballDirectionInstrListPtrs[v0 >> 1];
}
...
void Phantoon_Spawn8FireballsInCircleAtStart(uint16 k) {  // 0xA7D4A9
  ...
  if (v2 || v3) {
    ...
    if (!sign16(v5 - 8)) {
      ...
      EK->phant_var_E = 30;
      SpawnHardcodedPlm((SpawnHardcodedPlmArgs) { .x_pos = 0, .y_pos = 6, .plm_id_ = addr_kPlmHeader_B781_Phantoon_DrawDoorDuringFight });
    }
  }
}
...
void Phantoon_PickPatternForRound1(uint16 k) {  // 0xA7D596
  ...
  if (v2 || v3) {
    ...
    E1->phant_var_A = kPhantoon_EyeClosedTimers[(nmi_frame_counter_word >> 1) & 3];
    if ((NextRandom() & 1) != 0) {
      Enemy_Phantoon *E = Get_Phantoon(0);
      E->phant_var_C = 0;
      E->phant_var_B = 0;
...
void Phantoon_FadeOutDuringFireballRain(uint16 k) {  // 0xA7D7D5
  ...
  if (Get_Phantoon(0x40)->phant_var_F) {
    ...
    E->phant_var_E = kPhantoon_FireballRainHidingTimers[NextRandom() & 7];
  }
}
...
void Phantoon_SpawnRainingFireballs(uint16 k) {  // 0xA7D7F7
  ...
  if (v2 || v3) {
    ...
    int v4 = (8 * a) >> 1;
    Enemy_Phantoon *E0 = Get_Phantoon(0);
    E0->phant_var_A = kPhantoon_FlameRainSpawnPositions[v4];
    E0->base.x_pos = kPhantoon_FlameRainSpawnPositions[v4 + 1];
    E0->base.y_pos = kPhantoon_FlameRainSpawnPositions[v4 + 2];
...
void Phantoon_DyingPhantoonExplosions(uint16 k) {  // 0xA7D98B
  ...
  if (v2 || v3) {
    ...
    Enemy_Phantoon *E0 = Get_Phantoon(0);
    eproj_spawn_pt = (Point16U){ (int8)kPhantoon_DyingExplosionsTable[v5] + E0->base.x_pos, (int8)kPhantoon_DyingExplosionsTable[v5 + 1] + E0->base.y_pos };
...
PairU16 Phantoon_SetColorBasedOnHp_FindEntry(uint16 k) {  // 0xA7DC0F
  ...
  const uint16 *v1 = (const uint16 *)RomPtr_A7(r24 + kPhantoon_HealthPalettePtrs[r22]);
...
void Dachora_Func_1(uint16 j, uint16 k) {  // 0xA7F535
  ...
  memcpy(g_ram + kSpritePaletteRamPtrs[v3 >> 1], RomPtr_A7(j), 32);
}
...
int32 Dachora_Func_6(uint16 k) {  // 0xA7F6D5
  ...
  if ((int16)(E->dachor_var_A - g_word_A7F4D5) >= 0) {
    ...
    E->dachor_var_E = v2;
    if (!(uint8)v2) {
      Dachora_Func_1(kDachora_PalettePtrs_1[HIBYTE(E->dachor_var_E)], k);
...
void Dachora_Func_10(uint16 k) {  // 0xA7F90A
  Enemy_Dachora *E = Get_Dachora(k);
  Dachora_Func_1(kDachora_PalettePtrs_2[HIBYTE(E->dachor_var_E)], k);
